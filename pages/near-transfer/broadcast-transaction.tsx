...
export default function PageWithJSbasedForm() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const response = await fetch("/api/near-transfer/broadcast-transaction", {
      method: "POST",
      headers: {
  ...
  return (<>
    <div className="container">
      <h1 className={styles.title}>Submit Signed Transfer Txn to Broadcast</h1>
      ...
      <form onSubmit={handleSubmit} method="post">
        ...
        <select id="action" name="flowAction" required defaultValue="sign_transfer_tx">
          <option value="sign_transfer_tx">sign_transfer_tx</option>
          <option value="refresh_transfer_tx">refresh_transfer_tx</option>
        </select>
