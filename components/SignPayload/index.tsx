...
export default function SignPayload({ route, operation, description }: { route: string, operation: string, description: string }) {
    ...
    return (
        <div className="container">
            ...
            <form onSubmit={handleSubmit} method="post">
                ...
                <input type="text" id="transaction_payload" name="transaction_payload" value={txToSign} onChange={e => setTxToSign(e.target.value)} />
                <button disabled={!txToSign || appState.payload.signed} type="submit">Sign Transaction Payload</button>
            </form>
            ...
            {appState.payload.signed ? (
                <>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'flex-start', marginTop: '20px' }}>
                        <button className="prevPage" type="button" disabled={copied} onClick={() => copyText(appState.payload.signed)} style={{ marginRight: '12px' }}>{copied ? 'Copied!' : 'Copy'}</button>
