...
prev_page: 2_integration
next_page: 4_frontend
---
...
This is a demonstration of how the slate package simplifies transaction signing. 
...
- The incoming request body contains the unsigned transaction payload and the private key being used to sign it.

- It's important to point out here that at a minimum, it would be optimal for this action to occur entirely offline. Preferably on a well secured system, and using some kind of encrypted secrets vault such as [Hashicorp's Vault](https://www.vaultproject.io/). Once again, entirely for the purposes of this demo, we have chosen to greatly simplify this process to facilitate learning.

- The main takeaway here is that the transaction payload is being signed by the appropriate private key, and can then be passed to the Staking API to complete the flow.

- In the code above, once the payload is signed is is returned to the client-side as JSON.
